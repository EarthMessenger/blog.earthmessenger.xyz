...
其中 `bf` 是暴力进程，`a` 是需要对拍的进程，`gen_data` 是用 Python 写的数据生成
...
自动寻找目录下所有与可运行文档同名的 `.in` 文档进行评测，注意 TLE 会导致卡死。
...
UPD: 删除了 `dynamic_modint::bar()`，以此函数得 barrett 很慢。
...
`static_graph::operator[]` 用到了 `std::span`，需要 C++20，如果不支持可以考虑写
一个支持 `begin()`，`end()` 的简单 wrapper。

注：拷贝 `start` 于 `counter`，使其序与 `edges` 同。若无此要求，可改作：
...
又注：`static_graph` 虽减少内存碎片，但不能减少内存使用。构建之时，`edges`
`elist` 并存，用内存二倍。
