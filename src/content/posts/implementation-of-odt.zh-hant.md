...
`a[i] = v` 表示從 `i` 到 `i` 的下一個節點的下標這個區間對應的值都是 `v`。建議先
把 `a[0]` 和 `a[n]` 賦上值。
...
注意到你在 `split` 操作中沒有刪除節點，所以這樣迭代器不會失效，可以直接先
`split` 左節點再 `split` 右節點，非常優美。
...
這是 ODT 板子 [CF896C Willem, Chtholly and Seniorious][cf896c]。
