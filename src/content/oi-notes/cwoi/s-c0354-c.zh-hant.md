---
lang: zh-hant
opencc: true
tags: cwoi ds
title: CWOI S C0354 C 箱子
---

## 題意

```

```

## 解析

首先可以考慮一個樸素的暴力，前兩個詢問都非常好處理。對於第三個詢問，考慮求出其
差分陣列，根據差分計算每個位置作為端點產生的貢獻，再加上兩端的貢獻。

發現每個位置作為端點產生的貢獻只和他和前後兩個位置的差分有關。我們使用一個樹狀
陣列，下標 i 存的是第 i 個三元組和第 i+1 個三元組的詢問三的答案，減去兩端的貢獻。
然後如果要計算區間答案的話，顯然可以樹狀陣列區間求和。對於第一個修改操作，更新
修改位置前後的維護的答案即可。對於第二個修改，使用 `std::map` 維護顏色段，每次
更新顏色段端點處維護的答案即可。複雜度 $O(n\log n)$。

```cpp
#include <algorithm>
#include <array>
#include <iostream>
#include <map>
#include <string>
#include <vector>

void set_io(std::string name)
{
  std::cin.tie(nullptr);
  std::ios::sync_with_stdio(false);
}

constexpr int lowbit(int x) { return x & -x; }

struct fenwick
{
  int n;
  std::vector<long long> t;
  fenwick(int n) : n(n), t(n, 0) {}

  void add(int x, long long v)
  {
    for (x++; x <= n; x += lowbit(x)) t[x - 1] += v;
  }

  void set(int x, long long v)
  {
    add(x, v - sum(x, x + 1));
  }

  long long sum(int x) const
  {
    long long res = 0;
    for (; x > 0; x -= lowbit(x)) res += t[x - 1];
    return res;
  }

  long long sum(int l, int r) const
  {
    return sum(r) - sum(l);
  }
};

int main()
{
  set_io("box");

  int N, Q;
  std::cin >> N >> Q;
  std::vector<int> A(N), C(N), W(N);
  for (int i = 0; i < N; i++) {
    std::cin >> A[i] >> C[i] >> W[i];
  }

  std::map<int, int> color;
  for (int i = 0; i < N; i++) color[i] = C[i];
  auto split = [&color](int x)
  {
    auto p = --color.upper_bound(x);
    if (p->first == x) return p;
    auto v = p->second;
    return color.emplace(x, v).first;
  };
  auto get = [&color](int x)
  {
    auto p = --color.upper_bound(x);
    return p->second;
  };

  fenwick sum(N - 1);

  auto update_sum = [&sum, &A, &get, &W](int x)
  {
    if (get(x) == get(x + 1)) {
      int d = A[x + 1] - A[x];
      if (d < 0) {
        sum.set(x, (long long)W[x] * -d);
      } else {
        sum.set(x, (long long)W[x + 1] * d);
      }
    } else {
      sum.set(x, (long long)A[x] * W[x] + (long long)A[x + 1] * W[x + 1]);
    }
  };
  for (int i = 0; i + 1 < N; i++) update_sum(i);

  for (int i = 0; i < Q; i++) {
    int op;
    std::cin >> op;
    if (op == 1) {
      int x, v1, v2;
      std::cin >> x >> v1 >> v2;
      x--;
      A[x] = v1;
      W[x] = v2;
      if (x - 1 >= 0) update_sum(x - 1);
      if (x < N - 1) update_sum(x);
    } else if (op == 2) {
      int l, r, v;
      std::cin >> l >> r >> v;
      l--;
      auto lp = split(l);
      auto rp = split(r);
      std::vector<int> changed;
      for (auto p = lp; p != rp; ) {
        changed.emplace_back(p->first - 1);
        p = color.erase(p);
      }
      changed.emplace_back(r - 1);
      color.emplace(l, v);
      for (auto i : changed) {
        if (0 <= i && i < N - 1) {
          update_sum(i);
        }
      }
    } else if (op == 3) {
      int l, r;
      std::cin >> l >> r;
      l--;
      r--;
      long long ans = sum.sum(l, r);
      ans += (long long)A[l] * W[l];
      ans += (long long)A[r] * W[r];
      std::cout << ans << std::endl;
    }
  }
}
```
