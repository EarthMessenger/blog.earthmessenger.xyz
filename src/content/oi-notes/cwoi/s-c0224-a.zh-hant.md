---
lang: zh-hant
opencc: true
tags: cwoi math
title: CWOI S C0224 A 姨
---

題意使用 `openssl enc -aes256 -pbkdf2` 加密。

```

```

## 解析

大便題目，不取模是什麼 sb 操作。

分數的 $2^{R+C}$ 可以看作染黑的 $R$ 行 $C$ 列中選擇一個子集的所有方案。

列舉有 $i$ 行 $j$ 列被染黑，對答案的貢獻就是：

$$
\frac{\binom{n}{i}\binom{n}{j}\binom{m-t(i, j)}{k-t(i, j)}}{\binom{m}{k}}
$$

其中 $t(i, j) = i \times n + j \times n - i \times j$，即是染 $i$ 行 $j$ 列需要
染黑的格子數量。

注意到 $\binom{m-t(i, j)}{k-t(i, j)} = \binom{m-t(i, j)}{m-k}$，可以預處理出來。

可能需要高精度。

```python
from decimal import *
import functools

n, m, k = map(int, input().split())

fact = [1]
for i in range(1, n + 1):
    fact.append(fact[-1] * i)

@functools.cache
def binom(n, m):
    if m < 0 or m > n:
        return 0
    return fact[n] // fact[m] // fact[n - m]

@functools.cache
def t(i, j):
    return i * n + j * n - i * j

f = []

for i in range(m + 1):
    if i < m - k:
        f.append(0)
    elif i == m - k:
        f.append(1)
    else:
        f.append(f[i - 1] * i // (i - m + k))

s = ((f[m - t(i, j)] * binom(n, i) * binom(n, j) for i in
    range(n + 1)) for j in range(n + 1))

ans = sum((sum(i) for i in s))

tot = f[m]

getcontext().prec = 1000

ans = Decimal(ans) / Decimal(tot)

if ans >= pow(10, 99):
    print(pow(10, 99))
else:
    print(ans)

```

## 關於相對誤差

如果題目要求你的答案 $a$ 與標準答案 $b$ 的相對誤差不超過 $\epsilon$，實際上是要
求 $\frac{|a - b|}{b} \le \epsilon$，寫 checker 時一定要注意！
