...
首先考慮解決只有一個點的情況，求與 $u$ 距離不超過 $d$ 的點的數量，這個可以用點
分治離線解決。把距離 $u$ 不超過 $d$ 的點的數量和距離 $v$ 不超過 $d$ 的點的數量
加起來，再減去同時距離 $u$ 和 $v$ 都不超過 $d$ 的點的數量，就是答案。而求這個距
離 $u$ 和 $v$ 都不超過 $d$ 的點的數量可以轉化為距離 $u$ 和 $v$ 中點距離不超過某
個值的點數。為了保證中點是個點，需要先把每個邊中間插入一個點。
