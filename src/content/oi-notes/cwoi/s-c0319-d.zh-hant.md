...
首先，操作一個位置，一定是先除後乘的，不可能是先乘後除。

可以設計一個 dp，令 f(i, j) 表示考慮前 i 位，第 i 位的填的值是 j，最小代價是多
少。但是值域可能非常大，所以不可行。

一個非常人類智慧的最佳化。我們列舉一個分界點 p，表示 p 之前的位置填的值都是不小於
$2^{17}$ 的，而 p 之後的值都是大於等於 $2^{17}$ 的。對於 p 之前的位置，我們可以
通過上面講的 dp 進行計算，由於值域是 $2^{17}$，所以每個數經過操作有不超過
$17^2$ 種不同的值，複雜度 $O(17^2n)$。對於 p 之後的東西，首先，先通過一些操作將
序列對齊變成 17 位，每個數有不超過 17 種不同的值，然後如果在某個位置 $a_{i} >
a_{i+1}$，則 $a_{i+1}$ 後的所有值都必須乘二。容易設計一個 dp，令 g(i, j) 表示考
慮後 i 位，當前位置對齊到 $2^{17}$ 的值是 j。後面的部分複雜度 $O(17n)$。最後將
兩個 dp 的答案合併起來即可。
