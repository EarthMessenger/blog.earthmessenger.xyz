...
給定 12 根木棒的長度，每個木棒至多用一次，求最多組成多少個三角形。

首先我們用 $n^3$ 把三個木棒所有組合算出來，存到集合 $S$ 和 $T$ 中，分別表示可形成三角形的組合和不可形成三角形的組合。
$|S| + |T| = \binom{12}{3} = 1320$。令 $c = 1320$。

利用雙向搜尋的思想：$O(c^2)$ 處理出一個 $S$ 中元素和一個 $T$ 中元素無公共木棒的組合，存到 $U$，
同時處理出兩個 $S$ 中元素無公共木棒的組合，存到 $V$。此時已經可以判斷答案是否大於等於 1 或 2。
對於 3 或 4，列舉 $U$ 中元素，求出它的反元素，再在 $U$ 和 $V$ 中匹配。
