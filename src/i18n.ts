...
const uiZhHant = {
  ...
  '404.explain':
    '由於本博客經歷過多次更新和遷移，可能存在連接失效，可以在 GitHub 中搜索：',
  '404.search': '搜索',
} as const;

const uiZhHans = Object.fromEntries(
  Object.entries(uiZhHant).map(([key, value]) => [
    key,
    convertText(value, 'zh-hant', 'zh-hans'),
  ]),
);
...
export function useTranslations(lang: keyof typeof ui) {
  return function t(
    key: keyof (typeof ui)[typeof SITE_DEFAULT_LANG],
    ...
    return translation.replace(
      /\{(\w+)\}/g,
      (match: string, varName: string) => vars[varName] || match,
    );
  };
}
